    ...
    <div class="request-panel-address-bar">
      ...
      <div class="code-mirror-input-container">
        <CodeMirrorSingleLine v-model="activeTab.url" placeholder="Enter request URL"
          :key="'address-bar-' + activeTab._id" @keydown="handleAddressBarKeyDown" />
      </div>
      <button @click="sendRequest">Send</button>
    </div>
    <div class="request-panel-tabs">
      <div class="request-panel-tab"
        :class="{ 'request-panel-tab-active': activeRequestPanelTab === requestPanelTab.name }"
        @click="activeRequestPanelTab = requestPanelTab.name" v-for="requestPanelTab in requestPanelTabs">
        ...
        <template v-if="requestPanelTab.name === 'Body'">
          <template v-if="activeTab.body.mimeType === 'application/x-www-form-urlencoded'">
            <template
              v-if="'params' in activeTab.body && activeTab.body.params.filter(item => item.disabled === undefined || item.disabled === false).length > 0">
              <span>
                ({{
                    activeTab.body.params.filter(item => item.disabled === undefined || item.disabled === false).length
                }})
              </span>
            </template>
          </template>
        ...
        <template v-if="requestPanelTab.name === 'Query'">
          <template
            v-if="'parameters' in activeTab && activeTab.parameters.filter(item => item.disabled === undefined || item.disabled === false).length > 0">
            <span>
              ({{ activeTab.parameters.filter(item => item.disabled === undefined || item.disabled === false).length }})
            </span>
          </template>
        </template>
        <template v-if="requestPanelTab.name === 'Header'">
          <template
            v-if="'headers' in activeTab && activeTab.headers.filter(item => item.disabled === undefined || item.disabled === false).length > 0">
            <span>
              ({{ activeTab.headers.filter(item => item.disabled === undefined || item.disabled === false).length }})
            </span>
          </template>
        </template>
    ...
    <div class="request-panel-tabs-context">
      <div v-if="activeRequestPanelTab === 'Body'" class="request-panel-tabs-context-container">
        <select v-model="activeTab.body.mimeType" style="margin-bottom: 0.5rem"
          @change="bodyMimeTypeChanged($event.target.value)">
        ...
        <div v-if="activeTab.body.mimeType === 'application/x-www-form-urlencoded'">
          <table>
            <tr v-for="(param, index) in activeTab.body.params">
              <td>
                <input type="text" v-model="param.name" spellcheck="false" placeholder="name"
                  :disabled="param.disabled">
              </td>
              <td>
                <input type="text" v-model="param.value" spellcheck="false" placeholder="value"
                  :disabled="param.disabled">
              </td>
              <td>
                <input type="checkbox" :checked="param.disabled === undefined || param.disabled === false"
                  @change="param.disabled = $event.target.checked ? false : true">
              </td>
            ...
            <tr>
              <td colspan="4" style="text-align: center; user-select: none"
                @click="pushItem(activeTab.body, 'params', { name: '', value: '' })">
                + Add Item
              </td>
            </tr>
          </table>
        ...
        <div v-if="activeTab.body.mimeType === 'multipart/form-data'">
          <table>
                    ...
                    <label style="width: 100%; display: flex; align-items: center;">
                      <div :style="{ filter: !param.disabled ? undefined : 'opacity(0.4)' }">
                        ...
                        <span style="margin-left: 0.5rem">
                          <template v-if="param.files && param.files.length > 0">{{ param.files.length === 1 ?
                              param.files[0].name : `${param.files.length} files selected`
                          }}</template>
                          <template v-else>No File Selected</template>
                        </span>
                      </div>
                      <input type="file" @change="setFilesForParam($event.target.files, param)" multiple
                        :disabled="param.disabled" style="display: none;">
                    </label>
                  </template>
            ...
            <tr>
              <td colspan="4" style="text-align: center; user-select: none"
                @click="pushItem(activeTab.body, 'params', { name: '', value: '', type: 'text' })">
                + Add Item
              </td>
            </tr>
          </table>
        ...
        <div v-if="activeTab.body.mimeType === 'application/json'" class="oy-a">
          <CodeMirrorEditor v-model="activeTab.body.text" lang="json" class="code-editor"
            :key="'code-mirror-editor-' + activeTab._id + '-' + refreshCodeMirrorEditors" ref="jsonEditor">
          </CodeMirrorEditor>
        </div>
        ...
        <div style="display: grid; grid-template-rows: 1fr 130px auto; height: 100%; overflow: auto;"
          v-if="activeTab.body.mimeType === 'application/graphql'">
          <div class="oy-a" style="min-height: 130px;">
            <CodeMirrorEditor v-model="graphql.query" lang="graphql" class="code-editor"
              :key="'code-mirror-editor1-' + activeTab._id + '-' + refreshCodeMirrorEditors" ref="graphqlEditor">
            </CodeMirrorEditor>
          </div>
          <div style="margin-top: 0.5rem;display: grid; grid-template-rows: auto 1fr;">
            ...
            <div class="oy-a">
              <CodeMirrorEditor v-model="graphql.variables" lang="json" class="code-editor"
                :key="'code-mirror-editor2-' + activeTab._id + '-' + refreshCodeMirrorEditors" ref="jsonEditor">
              </CodeMirrorEditor>
            </div>
          </div>
        ...
        <div v-if="activeTab.body.mimeType === 'application/octet-stream'">
          <input type="file" @change="activeTab.body.fileName = $event.target.files[0]"
            style="width: 100%;  padding: 0.5rem; border: 1px solid var(--default-border-color);">
        </div>
      </div>
      <template v-if="activeRequestPanelTab === 'Query'">
        <table>
          ...
          <tr>
            <td colspan="4" style="text-align: center; user-select: none"
              @click="pushItem(activeTab, 'parameters', { name: '', value: '' })">
              + Add Item
            </td>
          </tr>
        </table>
      ...
      <template v-if="activeRequestPanelTab === 'Header'">
        <table>
          <tr v-for="(header, index) in activeTab.headers">
            <td>
              <input type="text" v-model="header.name" spellcheck="false" placeholder="name"
                :disabled="header.disabled">
            </td>
            <td>
              <input type="text" v-model="header.value" spellcheck="false" placeholder="value"
                :disabled="header.disabled">
            </td>
            <td>
              <input type="checkbox" :checked="header.disabled === undefined || header.disabled === false"
                @change="header.disabled = $event.target.checked ? false : true">
            </td>
          ...
          <tr>
            <td colspan="4" style="text-align: center; user-select: none"
              @click="pushItem(activeTab, 'headers', { name: '', value: '' })">
              + Add Item
            </td>
          </tr>
        </table>
      ...
      <template v-if="activeRequestPanelTab === 'Auth'">
        <select :value="activeTab.authentication?.type ?? 'No Auth'" @change="handleActiveTabAuthenticationTypeChange"
        ...
        <div v-if="activeTab.authentication && activeTab.authentication.type !== 'No Auth'">
          <table>
            <tr>
              ...
              <td style="width: 100%">
                <input type="checkbox"
                  :checked="activeTab.authentication.disabled === undefined || activeTab.authentication.disabled === false"
                  @change="activeTab.authentication.disabled = $event.target.checked ? false : true"
            ...
            <template v-if="activeTab.authentication.type === 'basic'">
              <tr>
                <td style="user-select: none;">
                  <label for="basic-auth-username"
                    :class="{ disabled: activeTab.authentication.disabled }">Username</label>
                </td>
                <td style="width: 100%">
                  <input type="text" v-model="activeTab.authentication.username" id="basic-auth-username"
                    :disabled="activeTab.authentication.disabled">
                </td>
              </tr>
              <tr>
                <td style="user-select: none;">
                  <label for="basic-auth-password"
                    :class="{ disabled: activeTab.authentication.disabled }">Password</label>
                </td>
                <td style="width: 100%">
                  <input type="text" v-model="activeTab.authentication.password" id="basic-auth-password"
                    :disabled="activeTab.authentication.disabled">
                </td>
              </tr>
            </template>
            <template v-if="activeTab.authentication.type === 'bearer'">
              <tr>
                ...
                <td style="width: 100%">
                  <input type="text" v-model="activeTab.authentication.token" id="basic-auth-token"
                    :disabled="activeTab.authentication.disabled">
                </td>
              </tr>
              <tr>
                ...
                <td style="width: 100%">
                  <input type="text" v-model="activeTab.authentication.prefix" id="basic-auth-prefix"
                    :disabled="activeTab.authentication.disabled">
                </td>
              </tr>
            </template>
      ...
      <template v-if="activeRequestPanelTab === 'Description'">
        <div style="height: 100%">
          <textarea v-model="activeTab.description" style="width: 100%; height: 100%; padding: 0.5rem;"
...
const activeRequestPanelTab = ref('Body')
const methods = ref([
  'GET',
  'POST',
  'PUT',
...
const refreshCodeMirrorEditors = ref(1)
...
watch(
  () => activeTab.value?.body.mimeType,
  () => {
    if (activeTab.value.body.mimeType === 'multipart/form-data') {
      if ('params' in activeTab.value.body) {
        // set type to text by default if type does not exist int he params array
        activeTab.value.body.params.forEach(param => {
...
watch(
  ...
  () => {
    ...
    activeTab.value.body.text = JSON.stringify({
...
function pushItem(object, key, itemToPush) {
...
function beautifyJSON() {
  try {
    jsonEditor.value.setValue(JSON.stringify(JSON.parse(activeTab.value.body.text), null, 4))
  } catch { } // catch all json parsing errors and ignore them
}
function beautifyGraphQL() {
  // TODO 美化GRAPHQL
  try {
    jsonEditor.value.setValue(JSON.stringify(JSON.parse(graphql.value.variables), null, 4))
  } catch { } // catch all json parsing errors and ignore them
}
function handleActiveTabAuthenticationTypeChange(event) {
  if ('authentication' in activeTab.value === false) {
    activeTab.value.authentication = {}
  }

  activeTab.value.authentication.type = event.target.value
}
function getAuthenticationTypeLabel(authenticationType) {
...
function bodyMimeTypeChanged(newMimeType) {
  let mimeType = null
  ...
  let contentTypeHeader = 'headers' in activeTab.value && activeTab.value.headers.find(header => header.name.toLowerCase() === 'content-type')

  if (contentTypeHeader) {
    contentTypeHeader.value = mimeType
  } else {
    if ('headers' in activeTab.value == false) {
      activeTab.value.headers = []
    }

    activeTab.value.headers.push({
      name: 'Content-Type',
      value: mimeType
    })
  }
...
function loadGraphql() {
  if (activeTab.value && activeTab.value.body.mimeType === 'application/graphql') {
...
function handleResponsePanelEmitter(event) {
  if (event === 'request restored') {
    loadGraphql()
    refreshCodeMirrorEditors.value++
  }
}
function setFilesForParam(files, param) {
  param.files = Array.from(files)
}

onMounted(() => {
  emitter.on('response_panel', handleResponsePanelEmitter)
})
onBeforeUnmount(() => {
  emitter.off('response_panel', handleResponsePanelEmitter)
})
</script>
...
.request-panel-tabs-context table th,
.request-panel-tabs-context table td {
  border: 1px solid var(--default-border-color);
  padding: 0.5rem;
}
